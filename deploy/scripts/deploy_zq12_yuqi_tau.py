...
class Deploy:
    ...
    def combine_obs_net(self, sin_pos, cos_pos, omega, eu_ang, pos, vel, action):
        self.obs_net[0, 0] = sin_pos
        self.obs_net[0, 1] = cos_pos
    ...
    def run_robot(self):
        ...
        try:
            ...
            while key_comm.listening:
                ...
                sin_pos = np.sin(2 * torch.pi * phase)
                cos_pos = np.cos(2 * torch.pi * phase)

                # 3.1 组合给神经网络的OBS, 其中action_net是上一帧神经网络生成的动作.其他值都经过缩放,P值还减去了默认姿势
                self.combine_obs_net(sin_pos, cos_pos, omega, eu_ang, pos_net, vel_net, action_net)
...
if __name__ == '__main__':
    ...
    if not args.load_model:
        args.load_model = f'{LEGGED_GYM_ROOT_DIR}/logs/zq/exported/policies/policy_5-6_nodelay_12dof.pt'
